...
class BaseLinearStateSpaceModel2(nn.Module):
    state_dim: int
    input_dim: int 
    output_dim: int
...
class LearnableLinearStateSpaceModel2(BaseLinearStateSpaceModel2):
    state_dim: int
    input_dim: int 
    output_dim: int
...
class RCModel2(BaseLinearStateSpaceModel2):
    ...
    def setup(self):
        ...
        #self._fxx = lambda x: fxx(self.Cai, self.Cwe, self.Cwi, self.Re, self.Ri, self.Rw, self.Rg)(x)
        self._fxu = self.fxu(self.Cai, self.Cwe, self.Cwi, self.Re, self.Rg)
        #self._fxu = lambda u: fxu(self.Cai, self.Cwe, self.Cwi, self.Re, self.Rg)(u)
        self._fyx = self.fyx()
        #self._fyx = lambda x: fyx()(x)
        self._fyu = self.fyu()
        #self._fyu = lambda u: fyu()(u)
...
# create model
model = RCModel2(name='RC', state_dim=3, input_dim=5, output_dim=1)
